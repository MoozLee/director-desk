...
配套 [skill 与离线工具](skills/director-desk/) 随软件提供，也可独立下载；没有 MCP 时，Agent 仍可生成可导入网页版本的 .director 工程文件。
