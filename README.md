...
用于 AI 短剧站位、走位、动作和运镜参考的三维预演工具。版本 0.3.7，支持浏览器和 Windows 桌面版。
...
官网为 https://bigthat.me 。网页与发布包由本机服务提供，通过独立 Cloudflare Tunnel 接入域名；不依赖 R2 或云主机，服务器关机时无法检查或下载更新。既有安装版需要先手动安装 0.3.7，才有软件内更新入口。免安装版不自动替换正在使用的目录。
