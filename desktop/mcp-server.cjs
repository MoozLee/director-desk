...
async function startMcp(definitions, call, version, { port, token }) {
    ...
    await new Promise((resolve, reject) => { server.once('error', e => reject(Error(e.code === 'EADDRINUSE' ? `MCP 端口 ${port} 已被占用，请关闭占用程序或其他导演台实例后重试；原连接配置保持不变` : '无法启动本机 MCP 服务'))); server.listen(port, '127.0.0.1', resolve); });
