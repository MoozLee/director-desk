...
import { DEFORM_TYPES, VISUAL_PRESETS } from '../visuals/model.ts';
...
export function createVisualPanel(ctx:AppContext){let page='animation',parameter='size',owner='',field:HTMLInputElement|null=null;
    const data=()=>{const e=ctx.current();return (ctx.inspectorTab==='deform'?e?.deform:e?.visual??e?.field??e?.warp??e?.deform) as unknown as Record<string,unknown>|undefined;};
    ...
    host.addEventListener('change',event=>{const target=event.target as HTMLInputElement;if(target===field){commit();return;}
        if(target.id==='visual-page'){page=target.value;ctx.renderInspector();return;}
    ...
    return {render(e:Entity){if(ctx.inspectorTab==='deform')e={...e,visual:undefined,field:undefined,warp:undefined};if(owner!==e.id){owner=e.id;parameter=e.visual?'size':e.field||e.warp?'strength':'amount';}const object=data();let html='<div class="surface-panel">';
        ...
        if(e.visual)html+=`<select id="visual-page" aria-label="元素属性">${options([['animation','动画与关键帧'],['generation','数量与出现时间'],['appearance','内容与外观']],page)}</select><p class="panel-help">${VISUAL_PRESETS[e.visual.preset]} · 固定种子，按戏段时间采样</p>`;
        ...
        if(e.visual){if(page==='appearance'&&e.visual.preset==='portal')html+=`<label class="field"><span>另一侧摄影机</span><select data-visual-setting="cameraId">${options([['','选择摄影机'],...ctx.project.entities.filter(x=>x.camera).map(x=>[x.id,x.name] as [string,string])],e.visual.cameraId??'')}</select></label>`;if(page==='generation')html+='<div class="field-pair">'+numeric('count','数量',1,50000)+numeric('seed','随机种子',0,2147483647)+'</div><div class="field-pair">'+numeric('start','开始 / 秒',0,86400)+numeric('end','结束（0 全程）',0,86400)+'</div>';
            if(page==='generation')html+=numeric('lifetime','循环寿命 / 秒',.1,1000);
            if(page==='appearance'&&['text','data'].includes(e.visual.preset))html+=`<label class="field"><span>内容</span><input type="text" data-visual-setting="text" maxlength="500" value="${escape(e.visual.text)}"/></label>`;
            if(page==='appearance')html+=`<div class="field-pair"><label class="field"><span>辅助颜色</span><input type="color" data-visual-setting="secondaryColor" value="${e.visual.secondaryColor}"/></label><label class="field"><span>细节质量</span><select data-visual-setting="quality">${options([['draft','草稿 · 1/4 粒子'],['normal','标准'],['high','完整 · 视图 2048']],e.visual.quality)}</select></label></div>`;
