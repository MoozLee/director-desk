...
export function createSurfacePanel(ctx:AppContext){
    let layerId='',page='mapping',lastEntity='',appearanceKey='transmission',activeInput:HTMLInputElement|null=null;
    ...
    host.addEventListener('change',event=>{const field=event.target as HTMLInputElement;if(field===activeInput){finish();return;}if(field.dataset.surfaceChoice)edit(()=>{const l=layer();if(!l)return;const k=field.dataset.surfaceChoice!;Object.assign(l,{[k]:['mesh','material'].includes(k)?Number(field.value):['unlit','loop','tile'].includes(k)?field.value==='true':field.value});});
        ...
        if(field.id==='surface-page'){page=field.value;ctx.renderInspector();}
    ...
    host.addEventListener('click',event=>{const target=(event.target as HTMLElement).closest<HTMLElement>('[data-surface-action]');if(!target)return;const action=target.dataset.surfaceAction;
        if(action==='page'){page=target.dataset.page!;ctx.renderInspector();return;}
    ...
    return {render(e:Entity){if(lastEntity!==e.id){lastEntity=e.id;layerId='';}const s=e.surface;if(!s?.layers.some(l=>l.id===layerId))layerId=s?.layers[0]?.id??'';const l=layer();
        ...
        const pages:[string,string][]=e.light?[['source','媒体来源'],['crop','裁剪范围'],['layout','位置与尺寸'],['play','播放时间'],['mapping','透明度']]:[['source','媒体来源'],['target','承载表面'],['mapping','映射与透明度'],['crop','裁剪范围'],['layout','位置与尺寸'],['play','播放时间'],['appearance','光学材质']];
        if(!pages.some(([key])=>key===page))page=pages[0][0];
        html+=`<div class="field-pair"><div class="surface-layer-select"><select id="surface-layer" aria-label="选择贴图层">${options((s?.layers??[]).map((l,i)=>[l.id,`${i+1} · ${ctx.project.media?.find(r=>r.id===l.resourceId)?.name??'媒体'}`]),layerId)}</select>${l?'<button data-surface-action="remove" aria-label="移除此贴图层" title="移除此贴图层">×</button>':''}</div><select id="surface-page" aria-label="材质设置页面">${options(pages,page)}</select></div>`;
        ...
        }else if(page==='crop'){html+='<div class="field-pair">'+input('crop.0','裁剪起点 X',l.crop[0],'.01','0','1')+input('crop.1','裁剪起点 Y',l.crop[1],'.01','0','1')+'</div><div class="field-pair">'+input('crop.2','裁剪宽度',l.crop[2],'.01','.001','1')+input('crop.3','裁剪高度',l.crop[3],'.01','.001','1')+'</div>';}
        else if(page==='layout'){html+='<div class="field-pair">'+input('offset.0','横向偏移',l.offset[0])+input('offset.1','纵向偏移',l.offset[1])+'</div><div class="field-pair">'+input('repeat.0','横向尺寸',l.repeat[0],'.1','.001')+input('repeat.1','纵向尺寸',l.repeat[1],'.1','.001')+'</div><div class="field-pair">'+input('rotation','旋转 / 度',l.rotation,'1','-36000','36000')+select('tile','超出区域',[['false','不重复'],['true','平铺重复']],String(l.tile??false))+'</div>';}
