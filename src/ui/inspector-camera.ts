...
export function cameraInspector(ctx: AppContext, e: Entity, navigation: InspectorNavigation) {
    ...
    const target = (c.targetPath ? '<p class="panel-help">已记录视线关键帧：独立看向目标及跟随机位优先使用关键帧注视点。到“运镜”编辑或清除；手动旋转及 POV 不使用它。</p>' : '') + select('机位方式', 'camera.mode', [['free', '独立机位 / 路径'], ['follow', '跟随对象'], ['pov', '绑定对象 / POV']], c.mode)
    ...
    const motion = button('look-open', `视线关键帧 · ${c.targetPath?.points.length ?? 0}`, '', 'wide subtle') + `<div class="action-grid">${['推近', '拉远', '横移', '升高', '环绕'].map(s => `<button data-motion="${s}">${s}</button>`).join('')}</div><p class="panel-help">运镜预设生成可编辑路径。到“路径”调整途经点、起止时间与路线形状。</p>`;
