...
export function createTimeline(ctx: AppContext) {
    function pathBar(e: Entity, left: (t: number) => number, width: (a: number, b: number) => number) {
        ...
        return pathSections(e.path!).map((part,index)=>`<div class="timeline-clip path ${isSelected({kind:'path',entityId:e.id,index})?'clip-selected':''}" data-path-id="${e.id}" data-section="${index}" style="--track-color:${e.color};left:${left(part.start)}%;width:${width(part.start,part.end)}%"><span>${e.kind==='camera'?'运镜':'路径'} ${index+1} · ${(part.end-part.start).toFixed(2)}s</span><i class="resize-handle" data-resize="true"></i></div>`).join(''); }
    function renderTimeline() {
        ...
        const left = (t: number) => Math.max(0, t / duration * 100);
        const width = (a: number, b: number) => Math.max(0, (b - a) / duration * 100);
        ...
        for (const e of ctx.project.entities.filter(e => e.kind !== 'prop' || e.path || e.clips.length)) {
            ...
            if (e.kind === 'camera' && e.camera!.mode !== 'free')
                ...
                for (const c of e.clips)
                    if (c.start < duration)
                        html += `<div class="timeline-clip ${isSelected({kind:'action',entityId:e.id,id:c.id})?'clip-selected':''}" data-entity="${e.id}" data-clip-id="${c.id}" style="--track-color:${e.color};left:${left(c.start)}%;width:${width(c.start, c.end)}%"><span>${escape(c.native ? ctx.engine.externalModels.inspection(ctx.project.resources!.find(r => r.id === e.external!.resourceId)!).animations.find(a => a.index === c.native!.index)?.name ?? clipLabel(c) : clipLabel(c))}</span><i class="resize-handle" data-resize="true"></i></div>`;
            html += '<div class="playhead-line"></div></div></div>';
            if ((e.kind === 'actor' || e.kind === 'crowd' || (e.kind === 'prop' && e.clips.length)) && e.path)
                html += `<div class="timeline-row path-row ${e.id === ctx.selected ? 'selected' : ''}"><button class="track-label sub-track" data-select="${e.id}">⌁ &nbsp; 运动路径</button><div class="track-lane">${pathBar(e, left, width)}<div class="playhead-line"></div></div></div>`;
        }
