...
export function createRetargetAnimationEditor(ctx: AppContext, refresh: () => void) {
    ...
    return { render(e: Entity) {
        ...
        return `<div class="retarget-editor" title="从工程模型选择动作，按当前人物骨长适配。来源映射和校正会独立保存；修改来源人物不会联动本片段。路径控制保留源旋转和上下起伏；片段外恢复原动作规则；衔接可混合相邻素材、原生动画与基础动作姿态。足部防穿透向上修正；支撑脚锁定按校准区间约束水平接触点，腿长不足时仍需调整路线和步幅。"><div class="native-row">${choose('retarget-source', '动作素材来源', sources.length ? sources.map(s => [s.id, s.label]) : [['', '先导入带骨架和动画的模型']], chosen)}<button class="subtle" data-retarget-action="add" ${sources.length ? '' : 'disabled'}>添加</button></div><div class="native-row">${choose('retarget-clip', '已排适配动作', clips.length ? clips.map(c => [c.id, `${escape(clipLabel(c))} · ${c.start.toFixed(2)}—${c.end.toFixed(2)} 秒`]) : [['', '尚未添加适配动作']], selected)}<button class="subtle" data-retarget-action="remove" ${clip ? '' : 'disabled'}>移除</button></div><div class="native-values">${choose('retarget-parameter', '调整素材参数', parameters, parameter)}${field}</div></div>`;
    } };
}
