...
export function createCurveEditor(ctx: AppContext) {
    ...
    function bind() {
        ...
        root.querySelector<HTMLSelectElement>('#curve-channel')!.onchange = event => { channel = (event.target as HTMLSelectElement).value; index = 1; ctx.renderInspector(); };
        root.querySelector<HTMLSelectElement>('#curve-segment')!.onchange = event => { index = Number((event.target as HTMLSelectElement).value); ctx.renderInspector(); };
        ...
        const getControls = (): [number, number, number, number] => {
            const circles = [...root.querySelectorAll<SVGCircleElement>('[data-curve-handle]')];
            return circles.flatMap(c => [(Number(c.getAttribute('cx')) - 24) / 252, (150 - Number(c.getAttribute('cy'))) / 126]) as [number, number, number, number];
        };
        const paint = (points: [number, number, number, number]) => { root.querySelector('#curve-canvas')!.innerHTML = graph({ bezier: points }); };
        root.onpointerdown = event => {
            const handle = (event.target as Element).closest<SVGCircleElement>('[data-curve-handle]');
        ...
        root.onpointerup = event => { if (!drag || drag.id !== event.pointerId) return; const edit = drag; drag = undefined; root.releasePointerCapture(event.pointerId); if (edit.revision !== ctx.scenes.context.revision) { ctx.toast('拖动期间工程已变化，请重新调整曲线'); ctx.renderInspector(); return; } commit({ bezier: edit.points }, edit.owner, edit.channel, edit.index); };
        root.onpointercancel = root.onlostpointercapture = () => { if (drag) { drag = undefined; ctx.renderInspector(); } };
        root.onkeydown = event => {
            if (event.key === 'Escape' && drag) { const id = drag.id; drag = undefined; root.releasePointerCapture(id); ctx.renderInspector(); return; }
            const handle = (event.target as Element).closest<SVGCircleElement>('[data-curve-handle]');
            ...
            const at = Number(handle.dataset.curveHandle) * 2 + axis; points[at] = Math.max(0, Math.min(1, points[at] + (['ArrowRight', 'ArrowUp'].includes(event.key) ? .01 : -.01))); commit({ bezier: points }); document.querySelector<SVGCircleElement>(`[data-curve-handle="${handle.dataset.curveHandle}"]`)?.focus();
        };
    }
