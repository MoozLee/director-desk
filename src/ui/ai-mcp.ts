...
export function mountAIMcp(panel: HTMLElement, status: (message: string) => void) {
    ...
    const descriptions = {
        http: '适合支持 Streamable HTTP 和请求头认证的本地 Agent。复制后合并到客户端的 MCP 配置。',
        'claude-code': '适合 Claude Code CLI 与桌面 Code 的本地会话。配置含 type: http，可合并到 .mcp.json；由该客户端读取并连接。',
    ...
    const refresh = (state: { enabled: boolean; url?: string }) => {
    ...
    copy.onclick = () => { if (bridge) safe(async () => {
        check(await bridge.copyMcp(client.value as Client));
        status('连接配置已复制，包含本机访问凭据，请只交给要连接的客户端。');
    }); };
    return refresh;
}
