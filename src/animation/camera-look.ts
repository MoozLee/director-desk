...
export interface CameraLookPath { smooth: boolean; points: Waypoint[] }
export function assertCameraLookPath(path: CameraLookPath | null | undefined) {
    if (path == null) return;
    if (!path || typeof path !== 'object' || Array.isArray(path) || Object.keys(path).some(k => !['smooth', 'points'].includes(k))
        || typeof path.smooth !== 'boolean' || !Array.isArray(path.points) || !path.points.length) throw Error('摄影机视线需要 smooth 和至少一个关键帧');
    path.points.forEach((p, i) => {
        if (!p || Object.keys(p).some(k => !['time', 'position'].includes(k)) || !Number.isFinite(p.time) || p.time < 0
...
/** World-space targets with bounded interpolation: smooth eases time, never overshoots a doorway. */
