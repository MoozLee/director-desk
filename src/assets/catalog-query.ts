...
export function queryAssetCatalog(query: AssetQuery = {}) {
    ...
    const list = searchAssets(query.query ?? '', query.group ?? '全部').filter(asset => {
        const available = capabilities(asset);
        return (!alternatives || alternatives.has(asset.id)) && (!query.ids || query.ids.includes(asset.id)) && (!query.kind || asset.kind === query.kind)
    ...
    return structuredClone({
        ...
        groups: ASSET_GROUPS.map(name => ({ name, count: ASSETS.filter(asset => asset.group === name).length })),
        missingIds: [...new Set(query.ids ?? [])].filter(id => !ASSETS.some(asset => asset.id === id)), assets, actions: ACTIONS,
