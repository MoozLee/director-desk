...
function patchEntity(target: Entity, value: Record<string, unknown> | undefined) {
    if (value && Object.hasOwn(value, 'camera') && target.kind === 'camera') {
        ...
        const allowedCamera = new Set(['aim', 'focal', 'target', 'targetId', 'targetHeight', 'mode', 'offset', 'inheritRotation', 'hideWalls', 'hiddenEntityIds']);
...
export function applyOperations(original: Project, operations: EditOperation[]): Project {
    ...
    for (const [index, op] of operations.entries()) {
        ...
        else if (op.operation === 'project') patch(project, op.patch, new Set(['name', 'duration', 'fps', 'aspect', 'room', 'floors', 'editorView']));
...
export function changeSummary(before: Project, after: Project) {
    ...
    return { hasChanges: JSON.stringify(before) !== JSON.stringify(after), added: after.entities.filter(e => !old.has(e.id)).map(e => ({ id: e.id, name: e.name })),
        ...
        projectChanged: ['name', 'duration', 'fps', 'aspect', 'room', 'cuts', 'production', 'floors', 'editorView', 'resources'].some(k => JSON.stringify(before[k as keyof Project]) !== JSON.stringify(after[k as keyof Project])) };
}
