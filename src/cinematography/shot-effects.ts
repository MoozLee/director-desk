import { DepthTexture, HalfFloatType, Mesh, OrthographicCamera, PerspectiveCamera, PlaneGeometry, Scene, ShaderMaterial, UnsignedIntType, Vector2, WebGLRenderer, WebGLRenderTarget } from 'three';
...
export class ShotEffects {
    ...
    render(renderer: WebGLRenderer, scene: Scene, camera: PerspectiveCamera, effects: CameraEffects | undefined, time: number, focus: number, labels: (camera: PerspectiveCamera) => void) {
        const blur = numberAt(effects?.channels?.blur, time), bloom = numberAt(effects?.channels?.bloom, time), lens = lensProjection(camera);
        if (!blur && !bloom && !lens) { renderer.render(scene, camera); labels(camera); return; }
