...
test('every built-in model responds to its own color without changing another instance', () => {
    for (const asset of ASSETS) {
        ...
        const colors = (root: T.Object3D) => { const out: string[] = []; root.traverse(o => { if (o instanceof T.Mesh) for (const m of Array.isArray(o.material) ? o.material : [o.material]) if ('color' in m) out.push((m.color as T.Color).getHexString()); }); return out; };
        ...
        disposeTree(original); disposeTree(recolored);
    }
});
