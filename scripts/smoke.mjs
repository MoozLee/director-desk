...
import { verifyEditing } from './verify-editing.mjs';
import { verifyTimeline } from './verify-timeline.mjs';
import { verifyBuilding } from './verify-building.mjs';
...
try {
  ...
  const savedProject=JSON.parse(await fs.readFile('tmp/smoke/roundtrip.director','utf8'));assert.equal(savedProject.entities.length,14);
  ...
  const extended = await verifyInteractions(page, savedProject, ids);
  console.log(JSON.stringify(extended));
  await verifyWorkspace(page, savedProject);
  await verifyEditing(page, savedProject);
  await verifyBuilding(page, savedProject);
  await verifyTimeline(page, savedProject);
