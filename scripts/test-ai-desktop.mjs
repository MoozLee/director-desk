...
try {
    ...
    await page.mouse.move(box.x + box.width / 2, box.y + 50); await page.mouse.down(); await page.mouse.move(668, box.y + 50); await page.mouse.up();
    assert.equal(Math.round((await page.locator('#ai-panel').boundingBox()).width), 320);
    for (const view of ['chat', 'settings', 'mcp']) await checkPanel(view);
    ...
    await page.mouse.move(freshBox.x + freshBox.width / 2, freshBox.y + 50); await page.mouse.down(); await page.mouse.move(550, freshBox.y + 50);
    await page.evaluate(() => window.dispatchEvent(new Event('blur')));
    await page.mouse.move(520, freshBox.y + 50); await page.mouse.up();
    assert.equal(Math.round((await page.locator('#ai-panel').boundingBox()).width), 400);
